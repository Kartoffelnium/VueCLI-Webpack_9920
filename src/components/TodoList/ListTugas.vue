<template>
 <v-main class="list">
   ...
   <v-card>
       ...
       <template v-slot:[`item.actions`]="{ item }">
         ...
         <v-btn small @click="deleteItem(item)">
          ...
          <template v-slot:[`item.checkbox`]="{ item }">
               <input type="checkbox" @change="onChange($event, item)">
       </template>
    ...
    <v-card>
        <ul v-for="(data, index) in selected" :key="index">
            <li>
                {{ data.task }}
            </li>
        </ul>
        <v-card-title>
        <v-btn v-if="selected.length" @click="deleteAll()">Delete All</v-btn>
        </v-card-title>
    </v-card>
 ...
 methods: {
   ...
   editItem(item){
    ...
    deleteAll(){    
        ...
        {
            this.todos.splice(this.selected[i], 1)
        }
        this.selected = []
    }
   },
